...
tf.flags.DEFINE_string(
    'host', '', 'Host of this server. Defaults to localhost')
tf.flags.DEFINE_integer(
    'max_num_faces', 4, 'Maximum number of faces to detect and translate.')
tf.flags.DEFINE_boolean(
    'debug', True, 'If true, use the mock twingan client for debugging.')
...
class MyHandler(HTTPServer.CGIHTTPRequestHandler):
    def __init__(self, req, client_addr, server):
        HTTPServer.CGIHTTPRequestHandler.__init__(
    ...
    def do_POST(self):
        ...
        if 'register_download' in form and form['register_download']:
            if 'subid' in form:
                ...
                subid_str = '0'
            print('TODO: do action for register download for id: %s and subid: %s' % (
                id_str, subid_str))
        ...
        if 'detectFace' in form and form['detectFace']:
            if 'image' not in form:
                self.post_server_internal_error(
                    'Missing image for detectFace mode.', id_str, {})
                return
            bin1 = form['image'][0]
            image_np = interface_utils.base64_to_numpy(
                bin1, contains_format=True)
            image_face_marked, face_found = FACE_DETECTOR.mark_face(
                image_path=None, image_np=image_np)
            self.post_success(id_str, {'image': interface_utils.numpu_to_base64(
                image_face_marked), 'face_found': face_found})
            return

        elif 'image' in form:
            bin1 = form['image'][0]
            input_image_path = interface_utils.save_encoded_image(
                bin1, os.getcwd() + '/static/images/inputs/' + id_str)

            cropped_image_pattern = os.getcwd() + '/static/images/cropped_faces/' + id_str + '_%d.png'
            faces = FACE_DETECTOR.crop_face_and_save(
                input_image_path, cropped_image_pattern)
            ...
            if num_faces == 0:
                if 'failOnMissingFace' in form and form['failOnMissingFace']:
                    ...
                    return
                shutil.copy(
                    input_image_path, os.getcwd() + '/static/images/cropped_faces/' + id_str + '_%d.png' % 0)
                faces = [util_io.imread(
                    input_image_path, (FLAGS.image_hw, FLAGS.image_hw))]
                num_faces = len(faces)

            transferred_image_file_format = os.getcwd() + '/static/images/transferred_faces/' + id_str + '_%d.png'
            ...
            if not succeed:
                self.post_server_internal_error(
                    'Domain transfer failed', id_str, {'num_faces': num_faces})
                return
            ...
            if 'do_waifu2x' in form:
                ...
                do_waifu2x = False
            if do_waifu2x:
                transferred_2x_image_file_format = os.getcwd() + '/static/images/transferred_faces_2x/' + \
                    id_str + '_%d.png'
                ...
                if not succeed:
                    self.post_server_internal_error(
                        'Waifu2x failed', id_str, {'num_faces': num_faces})
                    return
            ...
            combined_image_pattern = os.getcwd() + '/static/images/combined/' + id_str + '_%d.png'
            ...
            if not succeed:
                self.post_server_internal_error(
                    'Combine original and transferred failed.', id_str, {'num_faces': num_faces})
                return

            self.post_success(
                id_str, {'num_faces': num_faces, 'face_found': True})
        else:
    ...
    def combine_original_and_transferred(images, transferred_2x_image_file_format, combined_image_pattern):
        ...
        for i in range(len(images)):
            ...
            face_image = scipy.misc.imresize(
                images[i], (transferred_image.shape[0], transferred_image.shape[1]))
            combined_image = np.concatenate(
                (face_image, transferred_image), axis=1)
...
def main(_):
    ...
    httpd = HTTPServer.HTTPServer((FLAGS.host, FLAGS.port), MyHandler)
