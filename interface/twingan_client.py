...
class MockTwinGANClient(TwinGANClient):
  ...
  def __init__(self, hostport, image_hw, **kwargs):
    ...
    # self.mock_output_image = util_io.imread(os.getcwd() + '/static/images/mock/mock_twingan_output.png',
    #                                         shape=(image_hw,image_hw))

  def do_inference(self, output_dir, image_path=None, image_np=None):
    # invoke another py for create transferedImage by GAN
    command = "python3 " + os.getcwd() + "/inference/image_translation_infer.py --model_path='./model/256/'  --image_hw=256  --input_tensor_name='sources_ph' --output_tensor_name='custom_generated_t_style_source:0' --input_image_path='./static/images/cropped_faces/' --output_image_path='./static/images/transferred_faces/'"
    os.system(command)
...
if __name__ == '__main__':
  ...
  tf.flags.DEFINE_string('image_path', os.getcwd() + '/demo/web_interface_input/emma-watson-portrait.jpg', 'Path to the image to be translated.')
  tf.flags.DEFINE_string('output_dir', os.getcwd() + '/demo/face_translated/', 'Path to output directory for translated images.')
  tf.app.run()
